...
entropic_analysis <- function(ordered_vector,step_up=1,window_size,bins,verbose=FALSE,export_plots=FALSE,path=NULL)
{
  ...
  if (verbose)
  {
    ...
    barplot(entropy_clusters$silinfo$widths$sil_width,ylim=c(-0.3,1),ylab="Silhouette width",xlab="Clustered elements",main="K-means clustering quality")
  }
  if (export_plots)
  {
    if (is.null(path))
    ...
    if(!file.exists(path))
      dir.create(path)
    png(file=file.path(path,paste("Entropy - ",bins," bins - ",window_size," window size",".png", sep = "")))
    barplot(entropy_plotter,xlab=c("Granularity = ",step_up),ylab=c("Window size = ",window_size),main=c(bins," bins"),names.arg=seq(length(entropy_plotter))*step_up)
    dev.off()
    png(file=file.path(path,paste("Clustering quality - ",bins," bins - ",window_size," window size",".png", sep = "")))
    barplot(entropy_clusters$silinfo$widths$sil_width,ylim=c(-0.3,1),ylab="Silhouette width",xlab="Clustered elements",main="K-means clustering quality")
    dev.off()
  }
...
isolate_significant_elements <- function(ordered_vector,granularity=1,supervised=FALSE,process_log=FALSE,export_plots=FALSE,path=NULL)
{
  ...
  if (process_log)
  {
    ...
    progress <- txtProgressBar(max=80,char="=",style=3,title="Entropic analysis progress",label="testing")
  }
  for (i in 1:5)
    for (j in 5:20)
    {
      if (process_log)
      {
        cat("Reiterating for ",i*5," bins and a sliding window of ",j*10," features","\n",sep="")
      } else {
        setTxtProgressBar(progress,(i-1)*16+j-4)
      }
      suggested_surfaces <- c(suggested_surfaces,entropic_analysis(ordered_vector,step_up=granularity,bins=i*5,window_size=j*10,verbose=process_log,export_plots,path))
    }
  if (process_log)
  {
    par(mfrow=c(1,1))
...
entropic_ranks <- function(data_under_analysis,population_vector,data_origin=NULL,granularity=1,supervised=FALSE,process_log=FALSE,export_plots=FALSE,create_output_files=FALSE,is_logged=TRUE,logbase=2,huge_feature_list=FALSE)
{
  ...
  if (!supervised)
  {
    message("Trimming down the list of downregulated features without supervision...")
    ...
    message("Trimming down the list of upregulated features without supervision...")
    ...
    if (create_output_files)
    {
      if (dim(rank_product_lists$Table1)[1]>0)
        write.table(file="Downregulated list.txt",rank_product_lists$Table1[,3:5],sep="\t",quote=FALSE,row.names=TRUE,col.names=TRUE)
